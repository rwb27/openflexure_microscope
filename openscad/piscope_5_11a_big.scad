...
leg_r = 35; //radius of innermost part of legs
hole_r = 15; //size of hole in the stage
xy_lever_ratio = 1; //mechanical advantage of actuator over stage
z_lever_ratio = 2; //as above, for Z axis (must be >1)
...
z_strut_l = 25; //length of struts supporting Z carriage
...
module objective_clip_3(){
    ...
    top_w=2*(z_flexure_x-z_carriage_y);
	difference(){
		intersection(){
			union(){
				w1=z_carriage[0]*2;
                ...
                translate([0,objective_clip_y,0]) sequential_hull(){
					...
					translate([-top_w/2,dy,z_carriage[2]-z_carriage[1]]) cube([top_w,z_carriage[1],z_carriage[1]]);
				}
				//main body of the Z carriage
	...
	difference(){
		...
		union(){
			hull(){ //make it big enough to support legs and actuators
                ...
                reflect([1,0,0]) translate([-(leg_r-zflex_l)*1.41,0,0]) cube([d,d,h]);
			}
			...
			reflect([1,0,0]) translate([0,0,base_t-d]) sequential_hull(){
				...
				union(){
					translate([z_flexure_x,0,0]) cube([(leg_r-zflex_l)*1.41-z_flexure_x,d,z_flexure_spacing-base_t]);
//					leg_frame(-135) translate([-leg_outer_w/2,-zflex_l-2,0]) cube([d,2,d]);
	...
	hull(){
		translate([-7,8-(z_flexure_x-z_flex_w-d)-3,0]) cube([14,d,base_t+5]);
		translate([-7,camera_clip_y-7,0]) cube([14,d,10]);
	}

...
module objective_clip_3(){
	...
	difference(){
        ...
        translate([0,objective_clip_y,0]){
            rotate([45,0,0]) cube([999,1,1]*sqrt(2)*5,center=true); //slope up arms
            hull() reflect([0,0,1]) translate([0,0,5]) rotate([0,45,0]) cube([inner_w/sqrt(2),8,inner_w/sqrt(2)],center=true);
        }
	}
}
	...
	assign(h=base_t) difference(){
		union(){
            ...
            reflect([1,0,0]) sequential_hull(){
                ...
                translate([5+1,(leg_r-zflex_l-1)*sqrt(2)-5-1,0.5]) rotate([0,-6,45])cylinder(r=1,h=max(wall_h, z_flexure_x*0.15 + z_strut_t+4+4),$fn=8);
                translate([5+1,z_nut_y,0.5]) cylinder(r=1,h=wall_h,$fn=8);
                //SEE ALSO the bridge
            } 
		}
        ...
        //logo
        translate([z_flexure_x+3.1,0,10]) rotate([90-4,0,atan(((leg_r+actuating_nut_r-12)/sqrt(2))/((leg_r+actuating_nut_r+12)/sqrt(2)-z_flexure_x-3.1))]) scale([0.25,0.25,0.5]) logo_and_name();
	}
